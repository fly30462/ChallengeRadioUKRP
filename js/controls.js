...
radioplayer.controls = {
	...
	showPressPlayPrompt : function() {
		if (!radioplayer.controls.pressPlayPromptShowing) {
			var $pressPlayPrompt = $('<div class="point-prompt press-play-prompt">' + radioplayer.lang.controls.press_play_prompt + '</div>');
		...
		if (!radioplayer.consts.is_iOS) {
			...
			if (!$('#volume-controls').hasClass('showing-prompt')) {
				...
				$('#volume-controls').addClass('showing-prompt')
									 .append('<div class="point-prompt volume-prompt" id="volume-prompt">' + radioplayer.lang.controls.use_device_controls + '</div>');
